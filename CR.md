...
*   **第三公理（有效计算单位）：在本理论框架中，我们将标准模型中的基本粒子**定义**为宇宙“操作系统”进行计算的**最小功能单位**。每个粒子都拥有一个固定类型标签和一个**不会自发改变的、唯一的身份密钥 `K_particle`**。这个2个属性影响粒子与其他粒子的交互，即作为**计算规则`F`的核心参数**。(注意,这个定义是我们计算宇宙对真实的简化，根据我们理论内外划分相对性定义，实际上的粒子标签可以是粒子内部关系的外部表现)
